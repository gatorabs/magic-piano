...
def create_app(frames_dict, controls_dict, send_queue) -> Flask:
...
def start_flask_server(frames_dict, controls_dict, send_queue) -> None:
    """Inicializa o servidor Flask expondo os estados das teclas."""

    app = create_app(frames_dict, controls_dict, send_queue)
    app.run(
        host="0.0.0.0",
        port=5000,
        threaded=True,
